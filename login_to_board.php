	  ...
	  $rsChk = $DB->Execute($qryChk);
		  if (!$result) 	print $DB->ErrorMsg();	
			  ...
			  if( $rsChk->RecordCount()){	
				  ...
				  $errCode = "refresh";
				   mosRedirect("pinItem.php?item_id=$item_id");
			  }
			  else {
	...
	FB.login(function(response) {
		 if (response.authResponse) {
			   ...
			   FB.api('/me', function(response) {
				...
				// alert(response.name);
				 window.location = "<?php echo $_Config_live_site?>/login_to_board.php?id="+response.id+"&email="+response.email+"&fbName="+response.name+"&item_id="+<?php echo $item_id;?>;  
			   });
		 } 
